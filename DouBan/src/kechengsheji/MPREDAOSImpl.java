...
//			"search MN,MPRI,PTime,PText from MPRE left outer join Movie on MPRE.MNO=Movie.MNO left outer join MP on MPRE.MPNO=MP.MPNO ";
	private static final String SEARCH_MPRE_SQL = "select * from MPRE where MNO=";
	...
	public List<MPRE> getMPREs(String mno) {
		...
		try {
			...
			String finalsql = SEARCH_MPRE_SQL+"'"+mno+"'";
			rs = stmt.executeQuery(finalsql);
			while(rs.next()) 
			{
				MPRE mpre = new MPRE();
				mpre.setMNO(rs.getString("MNO"));
				mpre.setMPNO(rs.getString("MPNO"));
				mpre.setPTime(rs.getString("PTime"));
				mpre.setPText(rs.getString("PText"));
				MPREs.add(mpre);
			}
		}catch(Exception e) {
			e.printStackTrace();
